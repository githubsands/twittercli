...
func commandUsage(method string) {
	usage, err := twitterjson.MethodUsageText(method)
	if err != nil {
...
func main() {
	...
	method := args[0]
	usageFlags, err := twitterjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := twitterjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// twitterjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(twitterjson.Error); ok {
		...
		// The error is not a twitterjson.Error and this really should not
	...
	marshalledJSON, err := twitterjson.MarshalCmd("1.0", 1, cmd)
	if err != nil {
